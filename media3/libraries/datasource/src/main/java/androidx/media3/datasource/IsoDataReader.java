...
public final class IsoDataReader implements CacheDataReader, Closeable {
  ...
  private final LinkedHashMap<Long, byte[]> sectorCache =
      new LinkedHashMap<Long, byte[]>(CACHE_SECTORS * 2, 0.75f, true) {
        @Override
  ...
  public int read(long byteOffset, byte[] buf, int offset, int length) throws IOException {
    ...
    for (long s = firstSector; s <= lastSector; s++) {
      ...
      int sectorEnd =
          (int) (s == lastSector ? (byteOffset + length - 1) % SECTOR_SIZE + 1 : SECTOR_SIZE);
  ...
  private int directRead(long byteOffset, byte[] buf, int offset, int length) throws IOException {
    ...
    DataSpec spec =
        new DataSpec.Builder().setUri(uri).setPosition(byteOffset).setLength(length).build();
    try {
