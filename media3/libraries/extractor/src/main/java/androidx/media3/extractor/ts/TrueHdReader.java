...
public final class TrueHdReader implements ElementaryStreamReader {
  ...
  private void handleNonMajorPreamble() {
    boolean isMajorSync =
        (headerScratch[4] & 0xFF) == MAJOR_SYNC_B4
            && (headerScratch[5] & 0xFF) == MAJOR_SYNC_B5
            && (headerScratch[6] & 0xFF) == MAJOR_SYNC_B6
            && ((headerScratch[7] & 0xFF) == MAJOR_SYNC_B7_TRUEHD
                || (headerScratch[7] & 0xFF) == MAJOR_SYNC_B7_MLP);
    if (isMajorSync) {
      state = STATE_READING_HEADER;
    } else if ((headerScratch[0] & 0xFF) == AC3_SYNC_WORD_0
        && (headerScratch[1] & 0xFF) == AC3_SYNC_WORD_1) {
      ...
      int frameSize =
          (((headerScratch[0] & 0xFF) << 8) | (headerScratch[1] & 0xFF)) & FRAME_SIZE_MASK;
      frameSize *= 2;
  ...
  private boolean skipToMajorSync(ParsableByteArray pesBuffer) {
    ...
    for (int i = 0; i < totalBytes - (PREAMBLE_SIZE - 1); i++) {
      ...
      if (b4 == MAJOR_SYNC_B4
          && b5 == MAJOR_SYNC_B5
          && b6 == MAJOR_SYNC_B6
          && (b7 == MAJOR_SYNC_B7_TRUEHD || b7 == MAJOR_SYNC_B7_MLP)) {
  ...
  private boolean parseMajorSyncHeader() {
    int frameSize =
        ((((headerScratch[0] & 0xFF) << 8) | (headerScratch[1] & 0xFF)) & FRAME_SIZE_MASK) * 2;
    ...
    int chanmap5 = ((headerScratch[9] & 0x0F) << 1) | ((headerScratch[10] >> 7) & 1);
    int chanmap13 = ((headerScratch[10] & 0x1F) << 8) | (headerScratch[11] & 0xFF);
    int channelCount = channelCountFromMap(chanmap13 != 0 ? chanmap13 : chanmap5);
    if (channelCount <= 0) {
      channelCount = 2;
    }
    @Nullable
    String codecs = Ac3Util.isTrueHdAtmos(headerScratch) ? MimeTypes.CODEC_TRUEHD_ATMOS : null;
    ...
    sampleSize = frameSize;
    sampleDurationUs = C.MICROS_PER_SECOND * (SAMPLES_PER_AU << (ratebits & 7)) / sampleRate;
