...
/** Parses AV3A elementary streams and extracts individual frames. */
@UnstableApi
public final class Av3aReader implements ElementaryStreamReader {
  ...
  private static final int SYNC_SECOND_BYTE = 0xF2;
  ...
  public Av3aReader(@Nullable String language, @C.RoleFlags int roleFlags) {
    this(language, roleFlags, MimeTypes.VIDEO_MP2T);
  }

  Av3aReader(@Nullable String language, @C.RoleFlags int roleFlags, String containerMimeType) {
  ...
  private void findSync(ParsableByteArray source) {
    ...
    for (int i = start; i < end; i++) {
      if (lastByteWasFF && (data[i] & 0xFF) == SYNC_SECOND_BYTE) {
  ...
  private void readHeader(ParsableByteArray source) {
    ...
    frameSize = Av3aUtil.getFrameSize(header);
    ...
    output.sampleData(
        new ParsableByteArray(headerAccumulator, Av3aUtil.MAX_HEADER_SIZE),
        Av3aUtil.MAX_HEADER_SIZE);
