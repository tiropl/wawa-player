...
public final class MplsStreamEntry {
  ...
  /**
   * Coding type from the stream attributes block (matches CLPI stream coding type). E.g. 0x1B =
   * H.264, 0x24 = H.265/HEVC, 0x80 = LPCM, 0x81 = AC-3, 0x90 = PG subtitle.
   */
  public final int codingType;

  /**
   * 3-character ISO 639-2 language code (audio and subtitle streams only). Empty string when not
   * applicable (e.g. video streams).
   */
