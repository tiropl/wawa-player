...
/**
 * Cook/ATRAC audio uses GENR interleaving: sub_packet_h consecutive DATA packets form one decode
 * group that is de-interleaved before emission. FFmpeg formula: dst = sps * (h*x + ((h+1)/2)*(y&1)
 * + (y>>1))
 */
final class CookAudioReader implements TrackReader {
  ...
  CookAudioReader(
      TrackOutput trackOutput, int frameSize, int subPacketH, int subPacketSize, int sampleRate) {
  ...
  private void emitRawFrames(
      byte[] raw, int srcBase, int dataSize, long timestampUs, boolean isKeyFrame) {
  ...
  private void deinterleaveIntoAccumBuf(byte[] raw, int srcBase, int dataSize, int y) {
    ...
    for (int x = 0; x < framesPerRow; x++) {
      ...
      int dstOffset =
          subPacketSize * (subPacketH * x + ((subPacketH + 1) / 2) * (y & 1) + (y >> 1));
      if (x * subPacketSize + subPacketSize <= dataSize
          && dstOffset + subPacketSize <= accumBuf.length) {
  ...
  private void emitAccumulatedFrames() {
    ...
    for (int i = 0; i < totalFrames; i++) {
      ...
      long frameTs =
          (groupTimestampUs != C.TIME_UNSET) ? groupTimestampUs + (long) i * frameDurationUs : 0L;
      ...
      trackOutput.sampleMetadata(
          frameTs, (i == 0 && groupKeyFrame) ? C.BUFFER_FLAG_KEY_FRAME : 0, subPacketSize, 0, null);
    }
    groupTimestampUs = C.TIME_UNSET;
    groupKeyFrame = false;
  }
