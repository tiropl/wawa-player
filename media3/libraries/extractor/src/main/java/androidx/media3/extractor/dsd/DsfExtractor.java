...
public final class DsfExtractor implements Extractor {
  ...
  private int blockAlignPerChannel;
  ...
  public boolean sniff(ExtractorInput input) throws IOException {
    ...
    return headerBuffer.readLittleEndianInt() == MAGIC_DSD
        && headerBuffer.readLittleEndianLong() == DSD_CHUNK_SIZE;
  }
  ...
  private void parseHeader(ExtractorInput input) throws IOException {
    ...
    channelCount = headerBuffer.readLittleEndianInt();
    int sampleRateBits = headerBuffer.readLittleEndianInt();
    int bitOrder = headerBuffer.readLittleEndianInt();
    long sampleCount = headerBuffer.readLittleEndianLong();
    int blockAlign = headerBuffer.readLittleEndianInt();
    String mimeType =
        bitOrder == BIT_ORDER_LSBF
            ? MimeTypes.AUDIO_DSD_LSBF_PLANAR
            : MimeTypes.AUDIO_DSD_MSBF_PLANAR;
    blockAlignPerChannel = blockAlign;
    blockSize = blockAlign * channelCount;
    bytesPerSecond = sampleRateBits / 8 * channelCount;
    long durationUs = sampleCount * C.MICROS_PER_SECOND / sampleRateBits;
    headerBuffer.setPosition(OFFSET_DATA_SIZE);
    audioDataSize = headerBuffer.readLittleEndianLong() - CHUNK_HEADER_SIZE;
    audioSizeBytes =
        sampleCount > 0 ? Math.min((sampleCount / 8) * channelCount, audioDataSize) : audioDataSize;
    ...
    extractorOutput.seekMap(
        new DsdBitrateSeekMap(
            durationUs, bytesPerSecond, blockSize, AUDIO_DATA_OFFSET, C.LENGTH_UNSET, true));
    headerParsed = true;
  }
  ...
  private @ReadResult int readAudioBlock(ExtractorInput input) throws IOException {
    if (input.getPosition() >= AUDIO_DATA_OFFSET + audioDataSize
        || totalBytesOutput >= audioSizeBytes) {
      return RESULT_END_OF_INPUT;
    }
    long physicalRemaining = AUDIO_DATA_OFFSET + audioDataSize - input.getPosition();
    long validRemaining = audioSizeBytes - totalBytesOutput;
    if (physicalRemaining <= 0 || validRemaining <= 0) {
      return RESULT_END_OF_INPUT;
    }
    if (validRemaining < blockSize && physicalRemaining > validRemaining) {
      return readPaddedFinalBlock(input, physicalRemaining, validRemaining);
    }
    int toRead = (int) Math.min(blockSize, Math.min(physicalRemaining, validRemaining));
    if (toRead <= 0) {
      return RESULT_END_OF_INPUT;
    }
  ...
  private @ReadResult int readPaddedFinalBlock(
      ExtractorInput input, long physicalRemaining, long validRemaining) throws IOException {
    int physicalToRead = (int) Math.min(blockSize, physicalRemaining);
    if (physicalToRead <= 0 || blockAlignPerChannel <= 0 || channelCount <= 0) {
      return RESULT_END_OF_INPUT;
    }
    sampleBuffer.reset(physicalToRead);
    input.readFully(sampleBuffer.getData(), 0, physicalToRead);
    int validPerChannel = (int) Math.min(blockAlignPerChannel, validRemaining / channelCount);
    int sampleSize = 0;
    for (int channel = 0; channel < channelCount; channel++) {
      int channelOffset = channel * blockAlignPerChannel;
      if (channelOffset >= physicalToRead) {
        break;
      }
      int channelBytes = Math.min(validPerChannel, physicalToRead - channelOffset);
      if (channelBytes <= 0) {
        continue;
      }
      sampleBuffer.setPosition(channelOffset);
      trackOutput.sampleData(sampleBuffer, channelBytes);
      sampleSize += channelBytes;
    }
    if (sampleSize <= 0) {
      return RESULT_END_OF_INPUT;
    }
    long sampleTimeUs = sampleTimeUs();
    totalBytesOutput += sampleSize;
