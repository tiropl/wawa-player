...
public final class PlayItem {
  ...
  /**
   * Stream entries from the Stream Table Network (STN) within this play item. Contains
   * authoritative PID, codec type, and language information for all streams. May be empty if the
   * MPLS file does not include STN data.
   */
  ...
  public PlayItem(
      ...
      List<MplsStreamEntry> stnStreams) {
  ...
  public long getDurationUs() {
    return Util.scaleLargeTimestamp(
        outTimeTicks - inTimeTicks, C.MICROS_PER_SECOND, BdmvConstants.BDMV_TICKS_PER_SECOND);
  }
}
