...
public final class Ac3Reader implements ElementaryStreamReader {
  ...
  private int baseChannelMap;
  private int dependentChannelLocations;
            ...
            if (sampleSize == C.LENGTH_UNSET) {
              ...
              if (currentStreamType != SyncFrameInfo.STREAM_TYPE_TYPE1
                ...
                output.sampleMetadata(
                    pendingTimeUs, C.BUFFER_FLAG_KEY_FRAME, pendingTotalSampleSize, 0, null);
                pendingTotalSampleSize = 0;
              }
  ...
  public void endOfInputReached() {
    if (pendingTotalSampleSize > 0) {
      checkNotNull(output)
          .sampleMetadata(pendingTimeUs, C.BUFFER_FLAG_KEY_FRAME, pendingTotalSampleSize, 0, null);
      pendingTotalSampleSize = 0;
    }
  }
  ...
  private void parseHeader() {
    ...
    if (format != null) {
      ...
      } else if (MimeTypes.AUDIO_E_AC3.equals(lockedMime)
    ...
    if (currentStreamType != SyncFrameInfo.STREAM_TYPE_TYPE1) {
      if (MimeTypes.AUDIO_AC3.equals(frameInfo.mimeType)) {
        dependentChannelLocations = 0;
      }
      baseChannelCount = frameInfo.channelCount;
      baseChannelMap = frameInfo.channelMap;
      int channelCount = getChannelCount();
      if (format == null
          || channelCount != format.channelCount
          || frameInfo.sampleRate != format.sampleRate
          || !Objects.equals(mimeType, format.sampleMimeType)) {
    ...
    } else if (frameInfo.substreamId >= 0) {
      dependentChannelLocations |= frameInfo.channelMap;
      int channelCount = getChannelCount();
      if (format != null && channelCount > 0 && channelCount != format.channelCount) {
        format = format.buildUpon().setChannelCount(channelCount).build();
        output.format(format);
      }
    }
  }
  ...
  private int getChannelCount() {
    int channelMap = baseChannelMap | dependentChannelLocations;
    return channelMap != 0
        ? Ac3Util.getEac3ChannelCountFromChannelMap(channelMap)
        : baseChannelCount;
  }
}
