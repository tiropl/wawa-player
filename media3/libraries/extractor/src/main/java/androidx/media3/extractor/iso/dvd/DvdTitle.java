...
public final class DvdTitle {
  ...
  /**
   * Language codes for audio streams, indexed by sub-stream index (0x80–0x87 → index 0–7). Empty
   * string means not specified. Length is always 8.
   */
   ...
   * suitable for passing as {@code Format.initializationData[0]} to {@link VobsubParser}. Null if
   * not parsed.
   */
  @Nullable public final String vobsubIdx;

  /**
   * Audio format codes per stream (0=AC-3, 2=MPEG-1, 3=MPEG-2ext, 4=LPCM, 6=DTS, 7=SDDS). Length is
   * always 8; 0 for unused entries.
   */
  ...
  /**
   * Active audio sub-stream indices per PGC (values in range 0–7). A value of -1 means not
   * specified. Length is always 8.
   */
  public final int[] activeAudioStreams;

  /**
   * Active subpicture sub-stream indices per PGC (values in range 0–31). A value of -1 means not
   * specified. Length is always 32.
   */
  public final int[] activeSubpStreams;

  /**
   * VOB-set-relative sector numbers for each VOBU in this title's PGC, in order. Each entry
   * corresponds to the start sector of one VOBU. Used by DvdSourceHelper to build per-cell
   * IndexSeekMap instances for accurate seeking. May be empty if the VOBU address map was not
   * available.
   */
  public final long[] vobuSectors;

  /**
   * Chapter start times in microseconds, derived from PGC program map. {@code chapterTimesUs[0]} is
   * always 0 (start of title). {@code chapterTimesUs[i]} is the start time of chapter i (0-based).
