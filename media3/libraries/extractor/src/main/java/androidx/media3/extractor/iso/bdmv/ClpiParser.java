...
public final class ClpiParser {
  ...
  public static ClpiInfo parse(byte[] data, String clipName) throws IOException {
    ...
    if (programInfoAddr + PROGRAM_INFO_HEADER_SIZE <= data.length) {
      ...
      for (int p = 0; p < numPrograms && progPos + PROGRAM_ENTRY_HEADER_SIZE <= data.length; p++) {
        ...
        for (int s = 0; s < numStreams && streamPos + 3 <= data.length; s++) {
          ...
          if (streamType == BdmvConstants.STREAM_TYPE_HEVC
              && attrLength >= 5
              && attrPos + 6 <= data.length) {
            dynamicRangeType = (data[attrPos + 4] >> 4) & 0xF;
            colorSpace = data[attrPos + 4] & 0xF;
            hdrPlusFlag = (data[attrPos + 5] >> 7) & 1;
          }
          streams.add(
              new StreamInfo(pid, streamType, lang, dynamicRangeType, colorSpace, hdrPlusFlag));
    ...
    if (cpiAddr + 10 <= data.length) {
      ...
      if (cpiType == CPI_TYPE_EP_MAP) {
        ...
        for (int e = 0;
            e < numPidEntries && pidEntryPos + EP_MAP_PID_ENTRY_SIZE <= data.length;
            e++) {
    ...
    return new ClpiInfo(
        ...
        stcStartTime,
        spnStcStarts,
        stcPresentationStartTimes,
        stcPresentationEndTimes);
  }
  ...
  private static void parseEpEntries(
      byte[] data, int startAddr, int numCoarse, int numFine, List<EpMapEntry> out) {
    ...
    for (int c = 0; c < numCoarse; c++) {
      ...
      int nextRefFineId =
          (c + 1 < numCoarse)
              ? (int) ((bb.getInt(coarseBase + (c + 1) * EP_COARSE_ENTRY_SIZE) >> 14) & 0x3FFFFL)
              : numFine;
