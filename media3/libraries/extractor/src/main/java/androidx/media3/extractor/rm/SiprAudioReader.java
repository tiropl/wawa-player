 ...
 *
 * <p>FFmpeg ff_rm_parse_packet, DEINT_ID_SIPR: accumBuf[y * w .. y*w+w-1] receives packet_y (linear
 * accumulation), where w = audio_framesize and accumBuf size = w * sub_packet_h. After h packets,
 * ff_rm_reorder_sipr_data swaps nibble pairs per SIPR_SWAPS table (from FFmpeg rmsipr.c), then
 * ff_rm_retrieve_cache emits block_align-sized samples.
 */
final class SiprAudioReader implements TrackReader {
  ...
  SiprAudioReader(TrackOutput trackOutput, int subPacketH, int audioFrameSize, int blockAlign) {
    this.trackOutput = trackOutput;
    this.subPacketH = subPacketH;
    this.audioFrameSize = audioFrameSize;
    this.blockAlign = blockAlign > 0 ? blockAlign : audioFrameSize;
  ...
  public void consume(ParsableByteArray data, int dataSize, long timestampUs, boolean isKeyFrame) {
    if (audioFrameSize <= 0 || subPacketH <= 0 || blockAlign <= 0) {
  ...
  private void swapNibbles(int p, int q) {
    ...
    // Nibble index n maps to (buf[n >> 1] >> (4 * (n & 1))) & 0xF.
    int pShift = 4 * (p & 1);
    int qShift = 4 * (q & 1);
  ...
  private void emitAccumulatedData() {
    ...
    for (int offset = 0; offset + blockAlign <= totalSize; offset += blockAlign) {
      frameData.setPosition(offset);
      RmUtil.emitSample(trackOutput, frameData, blockAlign, ts, groupKeyFrame && offset == 0);
    }
    groupTimestampUs = C.TIME_UNSET;
    groupKeyFrame = false;
  }
