...
public final class PgsParser implements SubtitleParser {
  ...
  private static boolean isSupFileData(byte[] data, int offset, int length) {
    return length >= SUP_SEGMENT_HEADER_SIZE
        && (((data[offset] & 0xFF) << 8) | (data[offset + 1] & 0xFF)) == SUP_SEGMENT_MAGIC;
  }
  ...
  private void handleSupDisplaySetEnd(
      PgsSupTiming timing, ArrayList<Cue> displaySetCues, long displaySetStartTimeUs) {
