...
public final class BdmvConstants {
  ...
  static boolean isHdrDynamicRangeType(int dynamicRangeType) {
    return dynamicRangeType == DYNAMIC_RANGE_HDR10
        || dynamicRangeType == DYNAMIC_RANGE_DOLBY_VISION;
  }

  static boolean isAudioStreamType(int streamType) {
    return streamType == 0x03
        || streamType == 0x04
        || (streamType >= 0x80 && streamType <= 0x86)
        || streamType == 0xa1
        || streamType == 0xa2;
  }
