...
/**
 * Reassembles fragmented RealVideo packets into complete frames with FFmpeg-compatible slice
 * headers. Packet type is encoded in bits [7:6] of the first header byte: 0 = middle fragment, 1 =
 * complete frame, 2 = last fragment, 3 = complete frame in packet (type 3: len2 = frame size, pos =
 * timestamp ms; same output path as type 1)
 *
 * <p>Output format: (sliceCount-1)(1) + slice_table(8 * sliceCount) + frame_payload Each slice
 * table entry: valid_flag(4,LE) + byte_offset(4,LE)
 */
final class VideoReader implements TrackReader {
  ...
  private int expectedFrameLength = 0;
  ...
  private int consumeFrame(byte[] raw, int offset, int end, long timestampUs, boolean isKeyFrame) {
    ...
    if (type == 1) {
      ...
      handleFragmentedPacket(
          raw, offset, payloadLen, len2, type, seq, picNum, sliceHint, timestampUs, isKeyFrame);
    }
    return offset + payloadLen;
  }

  private void handleFragmentedPacket(
      byte[] raw,
      int offset,
      int payloadLen,
      int len2,
    ...
    if (isFirstSlice) {
      ...
      frameLen = 0;
      sliceCount = 0;
      expectedFrameLength = len2;
      sliceOffsets =
          (sliceOffsets == null || sliceOffsets.length < expectedSlices + 16)
              ? new int[expectedSlices + 16]
              : sliceOffsets;
    ...
    if (expectedFrameLength > 0 && frameLen + payloadLen > expectedFrameLength) {
      discardAssembledFrame();
      lastPicNum = -1;
      return;
    }
    ...
    frameLen += payloadLen;
    lastPicNum = picNum;
    if (type == 2 || (expectedFrameLength > 0 && frameLen == expectedFrameLength)) {
      emitAssembledFrame();
    }
  }

  private void emitSingleSliceFrame(
      byte[] raw, int offset, int payloadLen, long timestampUs, boolean isKeyFrame) {
    ...
    trackOutput.sampleMetadata(
        timestampUs, isKeyFrame ? C.BUFFER_FLAG_KEY_FRAME : 0, out.length, 0, null);
  }
  ...
  private void emitAssembledFrame() {
    ...
    trackOutput.sampleData(outputData, out.length);
    trackOutput.sampleMetadata(
        frameTimestampUs, frameKeyFrame ? C.BUFFER_FLAG_KEY_FRAME : 0, out.length, 0, null);
    sliceCount = 0;
    frameLen = 0;
    expectedFrameLength = 0;
  }

  private void discardAssembledFrame() {
    sliceCount = 0;
    frameLen = 0;
    expectedFrameLength = 0;
  }
  ...
  /**
   * FFmpeg get_num(): reads 2 bytes masked to 15 bits. If >= 0x4000: 15-bit value. Otherwise:
   * 30-bit value (shift 16 | next 2 bytes).
   */
