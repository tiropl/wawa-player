...
public final class DsdExtractor implements Extractor {
  ...
  private final ParsableByteArray sectorData =
      new ParsableByteArray(sectorBuffer, IsoConstants.SECTOR_SIZE);
  ...
  private void parseSector() {
    ...
    for (int i = 0; i < packetInfoCount; i++) {
      ...
      if (pktDataTypes[i] == DATA_TYPE_AUDIO
          && len > 0
          && offset + len <= IsoConstants.SECTOR_SIZE) {
        if (isDst) {
          if (pktFrameStarts[i]) {
  ...
  private void flushDstFrame() {
    ...
    trackOutput.sampleMetadata(
        sampleTimeUs, C.BUFFER_FLAG_KEY_FRAME, dstPendingFrameBytes, 0, null);
    dstPendingFrameBytes = 0;
    dstFrameCount++;
  }
  ...
  private void flushPendingSample() {
    if (isDst) {
      ...
      trackOutput.sampleMetadata(
          sampleTimeUs(), C.BUFFER_FLAG_KEY_FRAME, pendingOutputBytes, 0, null);
