...
public final class MplsParser {
  ...
  private static List<MplsStreamEntry> parseStn(
      byte[] data, ByteBuffer bb, int piBase, int piEnd, boolean isMultiAngle) {
    ...
    for (int g = 0; g < counts.length; g++) {
      ...
      for (int s = 0; s < counts[g]; s++) {
        ...
        sp = attrEnd;
        if (streamGroup == BdmvConstants.STREAM_GROUP_SEC_AUDIO
            || streamGroup == BdmvConstants.STREAM_GROUP_SEC_VIDEO) {
