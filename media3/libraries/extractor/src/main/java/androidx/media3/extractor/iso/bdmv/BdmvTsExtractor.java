...
public final class BdmvTsExtractor implements Extractor {
  ...
  @Nullable private final IndexSeekMap epSeekMap;
  private final BdmvTsPayloadReaderFactory payloadReaderFactory;
  private final int hdrDynamicRangeType;
  @Nullable private HdrUpgradeOutput hdrUpgradeOutput;

  public BdmvTsExtractor(
      @Nullable ClpiInfo clipInfo, long startM2ts, long inTimeTicks, long durationUs) {
  ...
  private static int findHdrDynamicRangeType(@Nullable ClpiInfo clipInfo) {
    ...
    for (StreamInfo s : clipInfo.streams) {
      if (BdmvConstants.isVideoStreamType(s.streamType)
          && BdmvConstants.isHdrDynamicRangeType(s.dynamicRangeType)) {
  ...
  @Nullable
  private static IndexSeekMap buildEpSeekMap(
      @Nullable ClpiInfo clipInfo, long startM2ts, long inTimeTicks, long durationUs) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      long virtualPos =
          (relativeM2ts / BdmvConstants.M2TS_PACKET_SIZE) * BdmvConstants.TS_PACKET_SIZE;
      ...
      if (stcIdx < clipInfo.stcPresentationStartTimes.length) {
        ...
        timeUs =
            cumulativeStcOffsetUs[stcIdx]
                + bdmvTicksToUs(entry.pts - stcStartTimeTicks)
                - inTimeOffsetUs;
      } else {
    ...
    return new IndexSeekMap(
        Arrays.copyOf(positions, count), Arrays.copyOf(timesUs, count), durationUs);
  }
  ...
  private static long[] buildCumulativeStcOffsets(ClpiInfo clipInfo) {
    ...
    for (int stcIdx = 1; stcIdx < numStc; stcIdx++) {
      long prevDurUs = 0;
      int prev = stcIdx - 1;
      if (prev < clipInfo.stcPresentationEndTimes.length
          && prev < clipInfo.stcPresentationStartTimes.length) {
        prevDurUs =
            Math.max(
                0,
                bdmvTicksToUs(
                    clipInfo.stcPresentationEndTimes[prev]
                        - clipInfo.stcPresentationStartTimes[prev]));
      }
  ...
  private static long getInTimeOffsetUs(
      @NonNull ClpiInfo clipInfo, long inTimeTicks, long[] cumulativeStcOffsetUs) {
    int inTimeStcIdx =
        clipInfo.stcPresentationStartTimes.length > 0
            ? Util.binarySearchFloor(clipInfo.stcPresentationStartTimes, inTimeTicks, true, true)
            : 0;
    if (inTimeStcIdx < clipInfo.stcPresentationStartTimes.length) {
      return cumulativeStcOffsetUs[inTimeStcIdx]
          + bdmvTicksToUs(inTimeTicks - clipInfo.stcPresentationStartTimes[inTimeStcIdx]);
    }
    return 0;
  }
  ...
  private static long bdmvTicksToUs(long ticks) {
    return Util.scaleLargeTimestamp(
        ticks, C.MICROS_PER_SECOND, BdmvConstants.BDMV_TICKS_PER_SECOND);
  }
  ...
  public void init(@NonNull ExtractorOutput output) {
    hdrUpgradeOutput =
        hdrDynamicRangeType > 0 ? new HdrUpgradeOutput(output, payloadReaderFactory) : null;
    ExtractorOutput hdrOutput = hdrUpgradeOutput != null ? hdrUpgradeOutput : output;
    if (epSeekMap != null) {
      delegate.init(
          new ForwardingExtractorOutput(hdrOutput) {
            @Override
  ...
  public void seek(long position, long timeUs) {
    if (hdrUpgradeOutput != null) {
      hdrUpgradeOutput.reset();
    }
  ...
  public @ReadResult int read(@NonNull ExtractorInput input, @NonNull PositionHolder seekPosition)
      throws IOException {
    @ReadResult int result = delegate.read(input, seekPosition);
    if (result == RESULT_END_OF_INPUT && hdrUpgradeOutput != null) {
      hdrUpgradeOutput.flush();
    }
    return result;
  }
