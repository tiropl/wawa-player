...
/* package */ final class PgsCueBuilder {
  ...
  private int parseCompositionObject(ParsableByteArray buffer, int remaining) {
    ...
    boolean cropped = (compositionFlag & COMPOSITION_OBJECT_CROPPED_FLAG) != 0;
    int cropX = 0;
    int cropY = 0;
    int cropWidth = 0;
    int cropHeight = 0;
    if (cropped) {
      ...
      cropX = buffer.readUnsignedShort();
      cropY = buffer.readUnsignedShort();
      cropWidth = buffer.readUnsignedShort();
      cropHeight = buffer.readUnsignedShort();
      remaining -= COMPOSITION_OBJECT_CROP_SIZE;
    }
    if (compositionObjectCount < MAX_COMPOSITION_OBJECTS && isPositionInsidePlane(x, y)) {
      PgsCompositionObject compositionObject = compositionObjects[compositionObjectCount++];
      compositionObject.set(objectId, windowId, x, y);
      if (cropped) {
        compositionObject.setCrop(cropX, cropY, cropWidth, cropHeight);
      }
    }
    return remaining;
  }
  ...
  void build(ArrayList<Cue> cues) {
    ...
    for (int i = 0; i < compositionObjectCount; i++) {
      ...
      Cue cue = buildCue(bitmapObject, palette.colors, compositionObject);
      if (cue != null) {
        cues.add(cue);
      }
    }
  ...
  private Cue buildCue(
      PgsBitmapObject bitmapObject, int[] colors, PgsCompositionObject compositionObject) {
    if (!decodeBitmapData(bitmapObject, colors)) {
      return null;
    }
    int sourceWidth = bitmapObject.width();
    int sourceHeight = bitmapObject.height();
    int bitmapWidth = sourceWidth;
    int bitmapHeight = sourceHeight;
    int sourceOffset = 0;
    if (compositionObject.cropped) {
      if (!isValidCrop(compositionObject, sourceWidth, sourceHeight)) {
        return null;
      }
      bitmapWidth = compositionObject.cropWidth;
      bitmapHeight = compositionObject.cropHeight;
      sourceOffset = compositionObject.cropY * sourceWidth + compositionObject.cropX;
    }
    if (bitmapWidth > planeWidth - compositionObject.x
        || bitmapHeight > planeHeight - compositionObject.y) {
      return null;
    }
    Bitmap bitmap =
        Bitmap.createBitmap(
    ...
    return createCue(bitmap, bitmapWidth, bitmapHeight, compositionObject.x, compositionObject.y);
  }

  private static boolean isValidCrop(
      PgsCompositionObject compositionObject, int bitmapWidth, int bitmapHeight) {
    return compositionObject.cropWidth > 0
        && compositionObject.cropHeight > 0
        && compositionObject.cropX < bitmapWidth
        && compositionObject.cropY < bitmapHeight
        && compositionObject.cropWidth <= bitmapWidth - compositionObject.cropX
        && compositionObject.cropHeight <= bitmapHeight - compositionObject.cropY;
  }

  private boolean decodeBitmapData(PgsBitmapObject bitmapObject, int[] colors) {
  ...
  private Cue createCue(Bitmap bitmap, int bitmapWidth, int bitmapHeight, int x, int y) {
