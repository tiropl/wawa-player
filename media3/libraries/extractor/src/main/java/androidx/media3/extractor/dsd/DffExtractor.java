...
public final class DffExtractor implements Extractor {
  ...
  private long dstFramesOutput;

  private static void readBytes(ExtractorInput input, ParsableByteArray buf, int count)
      throws IOException {
    buf.reset(count);
  ...
  private static boolean tryReadChunkHeader(ExtractorInput input, ParsableByteArray buf)
      throws IOException {
    try {
  ...
  private void parseHeader(ExtractorInput input) throws IOException {
    ...
    extractorOutput.seekMap(
        new DsdBitrateSeekMap(
            durationUs, bytesPerSecond, channelCount, bodyStart, bodyEnd, !isDst));
    headerParsed = true;
  }
  ...
  private @ReadResult int readDstFrame(ExtractorInput input) throws IOException {
    while (input.getPosition() + 12 <= bodyEnd) {
      ...
      if (id == ID_DSTF) {
        ...
        long sampleTimeUs =
            startTimeUs
                + Util.scaleLargeTimestamp(dstFramesOutput, C.MICROS_PER_SECOND, DST_FRAME_RATE_HZ);
        dstFramesOutput++;
