...
public final class FormatNameUtil {
  ...
  public static String getSampleMimeTypeDisplayName(Format format) {
    ...
    if (MimeTypes.APPLICATION_MEDIA3_CUES.equals(mimeType) && format.codecs != null) {
      mimeType = Util.splitAtFirst(format.codecs, " ")[0];
    }
    switch (mimeType) {
  ...
  private static String getAacDisplayName(Format format) {
    @Nullable
    String aacCodecs =
        MimeTypes.getCodecsCorrespondingToMimeType(format.codecs, MimeTypes.AUDIO_AAC);
    if (aacCodecs != null) {
  ...
  private static String getTrueHdDisplayName(Format format) {
    return MimeTypes.CODEC_TRUEHD_ATMOS.equals(format.codecs) ? "TrueHD + Atmos" : "TrueHD";
  }
  ...
  private static String getDtsHdMaDisplayName(Format format) {
    ...
    return MimeTypes.CODEC_DTS_HD_MA_X.equals(format.codecs) ? "DTS-HD MA + DTS:X" : "DTS-HD MA";
  }
