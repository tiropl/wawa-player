...
/** Fetches danmaku data for MGTV video page URIs. */
@UnstableApi
public final class MGTVFetcher implements Fetcher {
  ...
  private static final Pattern RE_NUXT_BLOCK =
      Pattern.compile("<script[^>]*>window\\.__NUXT__=([\\s\\S]{1,51200}?)</script>");
  private static final Pattern RE_DURATION =
      Pattern.compile("(\\d{1,2}:\\d{2}:\\d{2}|\\d+:\\d{2})");
  private static final String CDN_INFO_URL =
      "https://galaxy.bz.mgtv.com/getctlbarrage?version=8.1.39&abroad=0&os=10.0&platform=0&deviceid=5408f92e-8772-4f65-a8ec-1699b6764bcb&vid=%s&cid=%s";
  private static final String FALLBACK_SEGMENT_URL =
      "https://galaxy.bz.mgtv.com/cdn/opbarrage?vid=%s&cid=%s&time=%d";
  private static final ImmutableMap<String, String> HEADERS =
      FetcherUtil.headers("https://www.mgtv.com", "https://www.mgtv.com/");
  ...
  private static int parseDurationSec(String duration) {
  ...
  public Session prepare(Uri uri, OkHttpClient client, long durationMs) throws IOException {
    ...
    Matcher matcher = RE_URL.matcher(url);
    if (!matcher.find()) {
      throw new IOException("Not an MGTV URL: " + url);
    }
    String cid = matcher.group(1);
    String vid = matcher.group(2);
    int durationSec = (int) (durationMs / 1000L);
    if (durationSec <= 0) {
      String pageHtml = FetcherUtil.fetchString(client, url, HEADERS);
      durationSec = extractDuration(pageHtml);
      if (durationSec <= 0) {
        throw new IOException("Cannot extract duration from MGTV page: " + url);
      }
    }
    @Nullable String segmentBaseUrl = null;
    try {
      String cdnJson =
          FetcherUtil.fetchString(
              client, String.format(Locale.US, CDN_INFO_URL, vid, cid), HEADERS);
      segmentBaseUrl = parseCdnBase(cdnJson);
    } catch (IOException ignored) {
    }
    int segmentCount = (int) Math.ceil(durationSec / 60.0);
    return new MGTVSession(vid, cid, segmentCount, segmentBaseUrl, client);
  }
  ...
  private static final class MGTVSession implements Fetcher.Session {
    ...
    @Nullable private final String segmentBaseUrl;
    ...
    MGTVSession(
        ...
        String cid,
        int segmentCount,
        @Nullable String segmentBaseUrl,
        OkHttpClient client) {
      this.vid = vid;
      this.cid = cid;
    ...
    public List<Danmaku> fetchSegment(int segmentNumber) throws IOException {
      String segmentUrl =
          segmentBaseUrl != null
              ? segmentBaseUrl + "/" + (segmentNumber - 1) + ".json"
              : String.format(
                  Locale.US, FALLBACK_SEGMENT_URL, vid, cid, (long) (segmentNumber - 1) * 60_000L);
