...
/** Parses Tencent Video JSON danmaku data. */
@UnstableApi
public final class QQParser implements Parser {
  ...
  private static Danmaku parseItem(JSONObject item) {
    try {
      ...
      if (!styleJson.isEmpty()) {
        try {
          ...
          sourceGradientColors =
              ParserUtil.parseGradientHexColors(style.optJSONArray("gradient_colors"));
      ...
      return new Danmaku(text, timeMs, Danmaku.TYPE_SCROLL, color, 0f, sourceGradientColors);
