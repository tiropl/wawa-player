...
/** Fetches danmaku data for Bilibili video page URIs. */
@UnstableApi
public final class BiliFetcher implements Fetcher {
  ...
  private static final ImmutableMap<String, String> HEADERS =
      FetcherUtil.headers("https://www.bilibili.com", "https://www.bilibili.com/");
  ...
  public Session prepare(Uri uri, OkHttpClient client, long durationMs) throws IOException {
    ...
    Matcher matcher = RE_CID.matcher(pageHtml);
    if (!matcher.find()) {
      throw new IOException("Cannot extract cid from Bilibili page: " + uri);
    }
    return new BiliSession(matcher.group(1), client);
  }
  ...
  private static final class BiliSession implements Fetcher.Session {
    ...
    BiliSession(String cid, OkHttpClient client) {
      this.cid = cid;
      this.client = client;
    }
    ...
    public List<Danmaku> fetchSegment(int segmentNumber) throws IOException {
      byte[] data =
          FetcherUtil.fetchBytes(client, String.format(Locale.US, COMMENT_URL, cid), HEADERS);
