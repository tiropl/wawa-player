...
final class IsoChapterScanner {
  ...
  static ImmutableList<MediaChapter> buildBlurayPlaylistChapters(
      BdmvStructure bdmv, int playlistIndex) {
    ...
    for (ChapterMark mark : playlist.chapterMarks) {
      if (mark.markType != 1
          || mark.playItemRef < 0
          || mark.playItemRef >= playlist.playItems.size()) {
        continue;
      }
      ...
      result.add(
          MediaChapter.chapter(chapterIndex, timeUs, "Chapter " + (chapterIndex + 1), false));
      chapterIndex++;
    }
  ...
  private static long getPlaylistChapterTimeUs(List<PlayItem> playItems, ChapterMark mark) {
    ...
    long itemOffsetUs =
        Util.scaleLargeTimestamp(
            itemOffsetTicks, C.MICROS_PER_SECOND, BdmvConstants.BDMV_TICKS_PER_SECOND);
    if (itemOffsetUs < 0) {
      return C.TIME_UNSET;
    }
