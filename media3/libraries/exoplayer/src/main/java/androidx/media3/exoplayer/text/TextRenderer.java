...
public final class TextRenderer extends BaseRenderer implements Callback {
   ...
   * milliseconds.
   *
   * <p>The message payload must be a {@link Long}. A positive value delays subtitles, and a
   * negative value shows subtitles earlier.
   */
  ...
  private void renderFromCuesWithTiming(long positionUs) {
    ...
    if (outputNeedsUpdating) {
      ...
      long presentationPositionUs =
          previousCueChangeTimeUs == C.TIME_UNSET
              ? positionUs
              : getRendererPositionUs(previousCueChangeTimeUs);
  ...
  private void renderFromSubtitles(long positionUs) {
    ...
    if (textRendererNeedsUpdate) {
      ...
      long presentationTimeUs =
          getPresentationTimeUs(getRendererPositionUs(getCurrentEventTimeUs(subtitlePositionUs)));
  ...
  public boolean isReady() {
    ...
    if (isCuesWithTiming(checkNotNull(streamFormat))) {
      if (checkNotNull(cuesResolver)
              .getNextCueChangeTimeUs(getTextPositionUs(lastRendererPositionUs))
