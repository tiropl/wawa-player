...
final class BdmvSourceHelper {
  ...
  static MediaSource buildSourceFromStructure(
      MediaItem mediaItem,
      DataSource.Factory dataSourceFactory,
      Uri isoUri,
      UdfFileSystem udf,
      CacheDataReader isoReader,
      int editionIndex,
      BdmvStructure bdmv,
    ...
    if (main != bdmv.mainPlaylist) {
      clipInfos = loadClipInfos(isoReader, udf, main, clipInfos);
    }
    ...
    for (PlayItem item : main.playItems) {
      ...
      startM2ts =
          Math.min(
              m2tsEntry.length,
              (startM2ts / BdmvConstants.M2TS_PACKET_SIZE) * BdmvConstants.M2TS_PACKET_SIZE);
      endM2ts =
          Math.min(
              m2tsEntry.length,
              Util.ceilDivide(endM2ts, BdmvConstants.M2TS_PACKET_SIZE)
                  * BdmvConstants.M2TS_PACKET_SIZE);
      if (endM2ts <= startM2ts) {
        continue;
      }
      ...
      IsoDataSource.Factory clipFactory =
          new IsoDataSource.Factory(
              dataSourceFactory,
              m2tsEntry.extentOffsets,
              m2tsEntry.extentLengths,
              /* clipLogicalByteOffset= */ startM2ts,
              clipByteLength,
              /* stripM2tsHeaders= */ true,
      ...
      ExtractorsFactory bdmvFactory =
          () ->
              new Extractor[] {
                new BdmvTsExtractor(
                    clipInfo, capturedStartM2ts, capturedInTimeTicks, capturedDurationUs)
              };
      ProgressiveMediaSource.Factory psFactory =
          new ProgressiveMediaSource.Factory(clipFactory, bdmvFactory);
      psFactory.setLoadErrorHandlingPolicy(loadErrorHandlingPolicy);
      MediaItem clipItem =
          new MediaItem.Builder()
              .setUri(isoUri)
              .setCustomCacheKey(
                  IsoUtil.childCacheKey(mediaItem, "bdmv:" + item.clipName + ":" + startM2ts))
              .build();
  ...
  private static List<Playlist> synthesizePlaylistsFromStream(UdfFileSystem udf)
      throws IOException {
  ...
  private static List<Playlist> collectSortedPlaylists(
      CacheDataReader isoReader, UdfFileSystem udf, Map<String, ClpiInfo> clpiCache)
      throws IOException {
    ...
    for (IsoFileEntry mplsEntry : mplsEntries) {
      try {
        ...
        String name =
            mplsName.contains(".") ? mplsName.substring(0, mplsName.lastIndexOf('.')) : mplsName;
  ...
  private static Map<String, ClpiInfo> loadClipInfos(
      CacheDataReader isoReader,
      UdfFileSystem udf,
      Playlist playlist,
      Map<String, ClpiInfo> clpiCache) {
    ...
    for (PlayItem item : playlist.playItems) {
      ...
      try {
        clipInfos.put(
            item.clipName, ClpiParser.parse(readEntry(isoReader, clpiEntry), item.clipName));
      } catch (IOException ignored) {
      }
    }
    return clipInfos;
  ...
  static byte[] readEntry(CacheDataReader isoReader, IsoFileEntry entry) throws IOException {
    ...
    int pos = 0;
    for (int i = 0; i < entry.extentOffsets.length && pos < maxRead; i++) {
      int extentRead = 0;
      int extentLength = (int) Math.min(entry.extentLengths[i], maxRead - pos);
      if (entry.extentOffsets[i] == IsoFileEntry.UNRECORDED_EXTENT_OFFSET) {
        Arrays.fill(data, pos, pos + extentLength, (byte) 0);
        pos += extentLength;
        continue;
      }
      while (extentRead < extentLength) {
        int read =
            isoReader.read(
                entry.extentOffsets[i] + extentRead, data, pos, extentLength - extentRead);
        if (read <= 0) {
  ...
  @Nullable
  private static ClpiInfo getOrLoadClpi(
      CacheDataReader isoReader,
      UdfFileSystem udf,
      String clipName,
      Map<String, ClpiInfo> clpiCache) {
  ...
  private static void batchPrefetch(CacheDataReader isoReader, List<IsoFileEntry> entries) {
    ...
    for (IsoFileEntry e : entries) {
      for (int i = 0; i < e.extentOffsets.length; i++) {
        if (e.extentOffsets[i] == IsoFileEntry.UNRECORDED_EXTENT_OFFSET) {
          continue;
        }
        minOffset = Math.min(minOffset, e.extentOffsets[i]);
        maxEnd = Math.max(maxEnd, e.extentOffsets[i] + e.extentLengths[i]);
      }
    }
    ...
    if (span > 0 && span <= BATCH_PREFETCH_MAX_SPAN) {
      ...
      for (IsoFileEntry e : entries) {
        for (int i = 0; i < e.extentOffsets.length; i++) {
          ...
          isoReader.prefetchRange(e.extentOffsets[i], e.extentOffsets[i] + e.extentLengths[i]);
        }
      }
    }
  }
