...
final class IsoParsedMedia implements Closeable {
  ...
  @Nullable private final UdfFileSystem udf;
  @Nullable private final BdmvStructure bdmv;
  @Nullable private final DvdStructure dvd;
  @Nullable private final SacdStructure sacd;

  private IsoParsedMedia(
      MediaItem mediaItem,
      DataSource.Factory dataSourceFactory,
      Uri isoUri,
      IsoDataReader isoReader,
      ImmutableList<MediaEdition> editions,
      int defaultEditionIndex,
      @Nullable UdfFileSystem udf,
      @Nullable BdmvStructure bdmv,
      @Nullable DvdStructure dvd,
      @Nullable SacdStructure sacd) {
  ...
  @Nullable
  public static IsoParsedMedia parse(
      MediaItem mediaItem, DataSource.Factory dataSourceFactory, Uri isoUri, AtomicBoolean canceled)
      throws IOException {
    ...
    try {
      if (IsoUtil.isSacd(isoReader)) {
        ...
        success = true;
        return new IsoParsedMedia(
            mediaItem,
            dataSourceFactory,
            isoUri,
            isoReader,
            editions,
            defaultEditionIndex(editions),
            null,
            null,
            null,
            sacd);
      ...
      if (IsoUtil.isBluray(udf)) {
        ...
        ImmutableList<MediaEdition> editions =
            IsoEditionScanner.buildBlurayEditionsFromStructure(bdmv);
        success = true;
        return new IsoParsedMedia(
            mediaItem,
            dataSourceFactory,
  ...
  private static ImmutableList<MediaEdition> withSelected(
      ImmutableList<MediaEdition> editions, int selectedIndex) {
  ...
  public boolean canSelectEdition(int editionIndex) {
    ...
    if (sacd != null) {
      return (editionIndex == 0 && sacd.stereoArea != null)
          || (editionIndex == 1 && sacd.multiArea != null);
    } else if (bdmv != null) {
  ...
  public MediaSource buildSource(int editionIndex, LoadErrorHandlingPolicy loadErrorHandlingPolicy)
      throws IOException {
    int resolvedEditionIndex = resolveEditionIndex(editionIndex);
    if (sacd != null) {
      return SacdSourceHelper.buildSource(
          mediaItem,
          dataSourceFactory,
          isoUri,
          resolvedEditionIndex,
          sacd,
          loadErrorHandlingPolicy);
    } else if (bdmv != null) {
      return BdmvSourceHelper.buildSourceFromStructure(
          mediaItem,
          dataSourceFactory,
          isoUri,
          checkNotNull(udf),
      ...
      return DvdSourceHelper.buildSource(
          mediaItem, dataSourceFactory, isoUri, resolvedEditionIndex, dvd, loadErrorHandlingPolicy);
    }
